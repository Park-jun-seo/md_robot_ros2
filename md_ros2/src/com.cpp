...
{
    std::cout<<robotParamData.usb<<std::endl;
    try
    {
        port.open(robotParamData.usb);
        port.set_option(boost::asio::serial_port_base::baud_rate(robotParamData.nBaudrate));
...
int PutMdData(PID_CMD_t pid, uint16_t rmid, const uint8_t *pData, uint16_t length)
{
    ...
    {
        boost::asio::write(port, boost::asio::buffer(serial_comm_snd_buff, len));
    }
    return 1;
}

int MdReceiveProc(rclcpp::Node::SharedPtr node)
{
    uint8_t *pRcvBuf;
    uint8_t *pRcvData;
    uint8_t byRcvPID;
...
#if 0
            RCLCPP_INFO(node->get_logger(), "0x%x", curr_pid_robot_monitor2.sVoltIn);
            RCLCPP_INFO(node->get_logger(), "0x%x", curr_pid_robot_monitor2.byPlatStatus);
            RCLCPP_INFO(node->get_logger(), "0x%x", curr_pid_robot_monitor2.byDockingStatus);
            RCLCPP_INFO(node->get_logger(), "0x%x", curr_pid_robot_monitor2.head_motor_speed_ctrl_signal);
...
int AnalyzeReceivedData(const uint8_t byArray[], uint8_t byBufNum, rclcpp::Node::SharedPtr node) // Analyze the communication data
{
    uint8_t i, j;
    uint8_t data;
    ...
    for (j = 0; j < byBufNum; j++)
    {
        data = byArray[j];
#if 0
        RCLCPP_INFO(node->get_logger(),"%2d: %02x, %3d", j, data, data);
#endif
        switch (rcv_step)
        {
...
std::size_t available_bytes(boost::asio::serial_port& port)
{
    int bytes;
    ioctl(port.native_handle(), FIONREAD, &bytes);
    return static_cast<std::size_t>(bytes);
}

void ReceiveDataFromController(rclcpp::Node::SharedPtr node) // Analyze the communication data
{
    uint8_t byRcvBuf[250];
    uint8_t byBufNumber;

    byBufNumber = available_bytes(port);
    if (byBufNumber != 0)
    {
        ...
        boost::asio::read(port, boost::asio::buffer(byRcvBuf, byBufNumber));
        
        AnalyzeReceivedData(byRcvBuf, byBufNumber,node);
    }
